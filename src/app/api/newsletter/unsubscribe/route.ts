...
function page(title: string, body: string): Response {
  ...
  <body style="margin:0;background:#f6f1e7;font-family:Arial,sans-serif">
    <div style="max-width:480px;margin:12vh auto;background:#fffdf8;border:1px solid #ece3d2;border-radius:10px;padding:36px;text-align:center">
      ...
      <h1 style="font:600 20px/1.3 Georgia,serif;color:#241a0c;margin:22px 0 10px">${title}</h1>
...
export async function GET(req: Request) {
  ...
  try {
    const snap = await adminDb().collection("subscribers").where("email", "==", email).limit(1).get();
    if (!snap.empty) await snap.docs[0].ref.update({ status: "unsubscribed", unsubscribedAt: new Date().toISOString() });
  } catch (e) {
  ...
  return page("You're unsubscribed", `<b>${email}</b> won't receive any more newsletters from us. We're sorry to see you go.`);
}
